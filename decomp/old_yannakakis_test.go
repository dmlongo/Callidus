...
func samePossibleValues(node1 *Node, node2 *Node) bool {
	...
	if len(node1.Table.Tuples()) != len(node2.Table.Tuples()) {
		return false
	}
	for i := range node1.Table.Tuples() {
		if len(node1.Table.Tuples()[i]) != len(node2.Table.Tuples()[i]) {
			return false
		}
		for j := range node1.Table.Tuples()[i] {
			if node1.Table.Tuples()[i][j] != node2.Table.Tuples()[i][j] {
				return false
			}
		}
	}
...
func test1Data() (*Node, *Node, *Node, []csp.Solution) {
	...
	dInput := &Node{ID: 1, Table: db.InitializedRelation(dAttrs, dRel), Lock: &sync.Mutex{}}
	dInput.SetBag(dAttrs)

	rAttrs := []string{"Y", "Z", "U"}
	rRel := []db.Tuple{{3, 8, 9}, {9, 3, 8}, {8, 3, 8}, {3, 8, 4}, {3, 8, 3}, {8, 9, 4}, {9, 4, 7}}
	rInput := &Node{ID: 2, Table: db.InitializedRelation(rAttrs, rRel), Lock: &sync.Mutex{}}
	rInput.SetBag(rAttrs)

	sAttrs := []string{"Z", "U", "W"}
	sRel := []db.Tuple{{3, 8, 9}, {9, 3, 8}, {8, 3, 8}, {3, 8, 4}, {3, 8, 3}, {8, 9, 4}, {9, 4, 7}}
	sInput := &Node{ID: 3, Table: db.InitializedRelation(sAttrs, sRel), Lock: &sync.Mutex{}}
	sInput.SetBag(sAttrs)

	tAttrs := []string{"V", "Z"}
	tRel := []db.Tuple{{9, 8}, {9, 3}, {9, 5}}
	tInput := &Node{ID: 4, Table: db.InitializedRelation(tAttrs, tRel), Lock: &sync.Mutex{}}
	...
	dPartial := &Node{ID: 1, Table: db.InitializedRelation(dAttrs, dPartRel), Lock: &sync.Mutex{}}
	dPartial.SetBag(dAttrs)

	rPartRel := []db.Tuple{{3, 8, 9}, {9, 3, 8}, {8, 3, 8}, {3, 8, 3}}
	rPartial := &Node{ID: 2, Table: db.InitializedRelation(rAttrs, rPartRel), Lock: &sync.Mutex{}}
	rPartial.SetBag(rAttrs)

	sPartRel := []db.Tuple{{3, 8, 9}, {9, 3, 8}, {8, 3, 8}, {3, 8, 4}, {3, 8, 3}, {8, 9, 4}, {9, 4, 7}}
	sPartial := &Node{ID: 3, Table: db.InitializedRelation(sAttrs, sPartRel), Lock: &sync.Mutex{}}
	sPartial.SetBag(sAttrs)

	tPartRel := []db.Tuple{{9, 8}, {9, 3}, {9, 5}}
	tPartial := &Node{ID: 4, Table: db.InitializedRelation(tAttrs, tPartRel), Lock: &sync.Mutex{}}
	...
	dOutput := &Node{ID: 1, Table: db.InitializedRelation(dAttrs, dOutRel), Lock: &sync.Mutex{}}
	dOutput.SetBag(dAttrs)

	rOutRel := []db.Tuple{{3, 8, 9}, {3, 8, 3}}
	rOutput := &Node{ID: 2, Table: db.InitializedRelation(rAttrs, rOutRel), Lock: &sync.Mutex{}}
	rOutput.SetBag(rAttrs)

	sOutRel := []db.Tuple{{8, 3, 8}, {8, 9, 4}}
	sOutput := &Node{ID: 3, Table: db.InitializedRelation(sAttrs, sOutRel), Lock: &sync.Mutex{}}
	sOutput.SetBag(sAttrs)

	tOutRel := []db.Tuple{{9, 8}}
	tOutput := &Node{ID: 4, Table: db.InitializedRelation(tAttrs, tOutRel), Lock: &sync.Mutex{}}
...
func test2Data() (*Node, *Node, *Node, []csp.Solution) {
	...
	aInput := &Node{ID: 5, Table: db.InitializedRelation(aAttrs, aRel), Lock: &sync.Mutex{}}
	...
	bInput := &Node{ID: 6, Table: db.InitializedRelation(bAttrs, bRel), Lock: &sync.Mutex{}}
	...
	aPartial := &Node{ID: 5, Table: db.InitializedRelation(aAttrs, aPartRel), Lock: &sync.Mutex{}}
	...
	bPartial := &Node{ID: 6, Table: db.InitializedRelation(bAttrs, bPartRel), Lock: &sync.Mutex{}}
	...
	aOutput := &Node{ID: 5, Table: db.InitializedRelation(aAttrs, aOutRel), Lock: &sync.Mutex{}}
	...
	bOutput := &Node{ID: 6, Table: db.InitializedRelation(bAttrs, bOutRel), Lock: &sync.Mutex{}}
